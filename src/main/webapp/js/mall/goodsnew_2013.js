...
$(document).ready(function(){
	...
	$('#buynum_up').click(function(){
		...
		ntempvalue=parseInt(tempvalue+dizeng);
		if(ntempvalue<=kucun){$('#shuliang').val(ntempvalue)}else{$('#shuliang').val(kucun);Alert_open("ERROR|超出库存数量");}
	});
	
	/*购物数量 减一件*/
	$('#buynum_dw').click(function(){
		tempvalue=parseInt($('#shuliang').val());
		j=parseInt((tempvalue-qiding)/dizeng);
		ntempvalue=parseInt(qiding+(j-1)*dizeng);
		if(ntempvalue>=qiding){$('#shuliang').val(ntempvalue)}else{$('#shuliang').val(qiding);}
	});
